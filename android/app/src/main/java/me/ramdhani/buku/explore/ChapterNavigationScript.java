...
final class ChapterNavigationScript {
    private ChapterNavigationScript() {}
    ...
    static String source() {
        return "function cleanChapterText(value){" +
            ...
            "if(context){" +
            "var contextLabel=cleanChapterText((context.getAttribute('aria-label')||'')+' '+(context.getAttribute('title')||''));" +
            "if(/\\b(pagination|pages?)\\b/i.test(contextLabel)){return true;}" +
            "}" +
            "var value='';" +
            "var current=anchor;" +
            "for(var depth=0;current&&depth<3;depth+=1,current=current.parentElement){" +
            ...
            "});" +
            "if(paginationLabelAnchors.length>0){return uniqueChapterCandidate(paginationLabelAnchors);}" +
