...
describe('ExploreBrowserWorkflow', () => {
  let workflow: ExploreBrowserWorkflow;
  ...
  beforeEach(() => {
    ...
    workflow = new ExploreBrowserWorkflow({
      urlPolicy: TestBed.inject(BrowserUrlPolicy),
      sessionStore: store,
      viewport,
  ...
  it('initializes persisted tabs', async () => {
    ...
    expect(workflow.tabs()).toEqual(store.session.tabs);
    expect(workflow.activeTab()).toEqual(browserTab('tab-2', 'https://buku.example/'));
    expect(workflow.inputValue()).toBe('https://buku.example/');
    expect(workflow.lastUrl()).toBe('https://buku.example/');
  ...
  it('remembers the Library Series title on the active tab and persists the session', async () => {
    ...
    await workflow.initialize();
    await workflow.rememberActiveTabLibrarySeriesTitle('  Existing   Series  ');

    expect(workflow.activeTab()?.lastLibrarySeriesTitle).toBe('Existing Series');
  ...
  it('falls back to the first tab when the persisted selected tab is missing', async () => {
    ...
    expect(workflow.activeTab()?.id).toBe('tab-1');
    expect(workflow.inputValue()).toBe('https://example.com/');
  });
  ...
  it('initializes a persisted blank selected tab', async () => {
    ...
    expect(workflow.activeTab()).toEqual(browserTab('tab-1', null));
    expect(workflow.currentUrl()).toBeNull();
    expect(workflow.inputValue()).toBe('');
    expect(workflow.lastUrl()).toBeNull();
  });
  ...
  it('migrates legacy last URL into one selected tab when no tab session exists', async () => {
    ...
    expect(workflow.tabs()).toEqual([jasmine.objectContaining({ url: 'https://example.com/' })]);
    expect(workflow.activeTab()?.url).toBe('https://example.com/');
    expect(workflow.activeTab()?.backStack).toEqual([]);
    expect(store.writes[store.writes.length - 1]?.tabs).toEqual(workflow.tabs());
  });

  it('reports no secure or insecure state before a URL is loaded', () => {
    expect(workflow.isSecure()).toBeFalse();
    expect(workflow.isInsecure()).toBeFalse();
  });

  it('opens normalized URLs and clears validation errors', async () => {
    workflow.updateInputValue('not a url');
    await workflow.openInput();
    workflow.updateInputValue('example.com');

    const result = await workflow.openInput();

    expect(result.ok).toBeTrue();
    expect(workflow.validationError()).toBeNull();
    expect(workflow.currentUrl()).toBe('https://example.com/');
  ...
  it('keeps invalid input on the landing page with an inline validation error', async () => {
    workflow.updateInputValue('reader search');

    const result = await workflow.openInput();

    expect(result.ok).toBeFalse();
    expect(workflow.validationError()).toBe('Enter a URL, not search terms.');
  ...
  it('resumes the most recent URL tab', async () => {
    ...
    const result = await workflow.resumeLastUrl();
  ...
  it('does not resume missing or blank tabs', async () => {
    ...
    expect((await workflow.resumeTab('missing-tab')).ok).toBeFalse();
    expect((await workflow.resumeTab('tab-2')).ok).toBeFalse();
    expect(viewport.loadedUrls).toEqual(['https://example.com/']);
  });

  it('opens landing-page URLs in a new selected tab', async () => {
    await workflow.initialize();
    workflow.updateInputValue('buku.example');

    const result = await workflow.openInputInNewTab();
    viewport.emit({
      type: 'navigation',
      committed: true,
      state: {
    ...
    expect(result.ok).toBeTrue();
    expect(workflow.activeTab()?.url).toBe('https://buku.example/');
    expect(workflow.tabs().length).toBe(2);
    expect(viewport.loadedUrls).toEqual(['https://buku.example/']);
  });
  ...
  it('updates the active tab URL from browser-page submissions', async () => {
    ...
    expect(result.ok).toBeTrue();
    expect(workflow.tabs().filter((tab) => tab.url !== null).length).toBe(1);
    expect(workflow.activeTab()?.url).toBe('https://buku.example/');
  });
  ...
  it('switches tabs by loading the selected tab URL', async () => {
    ...
    await workflow.selectTab('tab-2');

    expect(workflow.activeTab()?.id).toBe('tab-2');
    expect(viewport.loadedUrls).toEqual(['https://example.com/', 'https://buku.example/']);
  });
  ...
  it('creates a selected blank tab', async () => {
    ...
    await workflow.createBlankTab();

    expect(workflow.activeTab()?.url).toBeNull();
    expect(workflow.inputValue()).toBe('');
    expect(workflow.currentUrl()).toBeNull();
    expect(viewport.hideCount).toBe(1);
    expect(viewport.destroyCount).toBe(1);
  ...
  it('ignores unknown tab selection and close requests', async () => {
    ...
    await workflow.selectTab('missing-tab');
    await workflow.closeTab('missing-tab');
  ...
  it('closing the first active tab clears the viewport when the next tab is blank', async () => {
    ...
    await workflow.closeTab('tab-1');

    expect(workflow.activeTab()).toEqual(browserTab('tab-2', null));
    expect(workflow.inputValue()).toBe('');
    expect(workflow.currentUrl()).toBeNull();
    expect(viewport.hideCount).toBe(1);
    expect(viewport.destroyCount).toBe(1);
  ...
  it('closing the final tab creates a selected blank tab', async () => {
    ...
    expect(workflow.tabs()).toEqual([jasmine.objectContaining({ url: null })]);
    expect(workflow.activeTab()?.url).toBeNull();
    expect(workflow.inputValue()).toBe('');
  });
  ...
  it('ignores committed navigation URL persistence when no tab is selected', () => {
    expect(workflow.activeTab()).toBeNull();
  ...
  it('updates state and persists committed navigation events', async () => {
    await workflow.initialize();
    workflow.updateInputValue('example.com');
    await workflow.openInputInNewTab();
    viewport.emit({
      type: 'navigation',
      committed: true,
      state: {
    ...
    expect(workflow.currentUrl()).toBe('http://example.com/');
    expect(workflow.isInsecure()).toBeTrue();
    expect(workflow.canGoBack()).toBeTrue();
  ...
  it('falls back to no tab title until a page load commits', async () => {
    ...
    expect(workflow.activeTab()?.url).toBeNull();
    expect(workflow.activeTab()?.pageTitle).toBeNull();
    ...
    expect(workflow.activeTab()).toEqual(
  ...
  it('waits for address-bar navigation to commit before appending to the stack', async () => {
    ...
    await workflow.initialize();
    workflow.updateInputValue('two.example');

    await workflow.openInput();

    expect(workflow.activeTab()).toEqual(browserTab('tab-1', 'https://one.example/'));
  ...
  it('caps each tab back stack at 25 entries', async () => {
    ...
    expect(workflow.activeTab()?.backStack.length).toBe(25);
    expect(workflow.activeTab()?.backStack[0]).toBe('https://page-5.example/');
    expect(workflow.activeTab()?.backStack[24]).toBe('https://page-29.example/');
  });
  ...
  it('uses persisted fallback back when native history is empty', async () => {
    ...
    const result = await workflow.goBack();
  ...
  it('continues through the persisted stack after a restart fallback creates native history', async () => {
    ...
    expect(workflow.canGoBack()).toBeFalse();
  });
  ...
  it('controls reload, stop, back, and forward through the viewport', async () => {
    ...
    await workflow.stopOrReload();
    await workflow.stopOrReload();
    const backResult = await workflow.goBack();
    await workflow.goForward();
  ...
  it('shows load failure notices and can retry the current URL', async () => {
    ...
    const result = await workflow.retryCurrentUrl();

    expect(result.ok).toBeTrue();
    expect(viewport.loadedUrls).toEqual(['https://example.com/']);
    expect(workflow.notice()?.message).toBe('Page failed to load: Network error');
  });
  ...
  it('copies and opens the current URL externally', async () => {
    ...
    await workflow.copyCurrentUrl();
    await workflow.openCurrentUrlExternally();

    expect(viewport.copiedUrl).toBe('https://example.com/');
    expect(opener.openedUrl).toBe('https://example.com/');
    expect(workflow.notice()?.kind).toBe('copied');
  });
  ...
  it('dismisses notices and reports secure state', () => {
    ...
    expect(workflow.isSecure()).toBeTrue();
  ...
  it('shows, hides, and closes the native viewport', async () => {
    ...
    await workflow.showViewport(rect);
    await workflow.hideViewport();
  ...
  it('extracts an article, stores it in memory, and hides the native viewport', async () => {
    ...
    const result = await workflow.openReadingMode();
  ...
  it('does not try reading mode without a current URL or while loading', async () => {
    expect((await workflow.openReadingMode()).ok).toBeFalse();
  ...
  it('shows a notice when reading mode is unavailable', async () => {
    ...
    expect(result.ok).toBeFalse();
    expect(workflow.readingArticle()).toBeNull();
    expect(workflow.notice()).toEqual({
  ...
  it('closes reading mode without destroying the browser session', async () => {
    ...
    await workflow.openReadingMode();
    workflow.closeReadingMode();

    expect(workflow.readingModeActive()).toBeFalse();
    expect(workflow.readingArticle()).toEqual(articleSnapshot);
    expect(workflow.currentUrl()).toBe('https://example.com/article');

    workflow.discardReadingMode();
  ...
  it('reopens a retained article without extracting again', async () => {
    ...
    expect(workflow.readingModeActive()).toBeTrue();
    expect(workflow.readingArticle()).toEqual(articleSnapshot);
    expect(viewport.extractCount).toBe(1);
  });
  ...
  it('opens reader links through the Explore Browser session', async () => {
    ...
    const result = await workflow.openReadingModeLink('/next');
  ...
  it('keeps the reader open when reader links use unsupported schemes', async () => {
    ...
    const result = await workflow.openReadingModeLink('mailto:reader@example.com');
  ...
  it('keeps the reader open when reader links cannot be resolved', async () => {
    ...
    const result = await workflow.openReadingModeLink('https://[');
  ...
  it('ignores reader link navigation without an in-memory article', async () => {
    const result = await workflow.openReadingModeLink('https://example.com/next');
  ...
  it('loads and replaces the in-memory article for next chapter navigation', async () => {
    ...
    const resultPromise = workflow.navigateReadingChapter('next');

    expect(workflow.chapterNavigationLoading()).toBeTrue();
    expect(workflow.readingArticle()).toEqual(articleSnapshotWithChapters);
    ...
    expect(workflow.chapterNavigationLoading()).toBeFalse();
    expect(workflow.readingArticle()).toEqual(nextArticle);
    expect(viewport.loadedUrls).toEqual(['https://example.com/next']);
  ...
  it('loads and replaces the in-memory article for previous chapter navigation', async () => {
    ...
    const resultPromise = workflow.navigateReadingChapter('previous');
    viewport.emit({
      type: 'navigation',
      committed: true,
      state: {
    ...
    expect(workflow.readingArticle()).toEqual(previousArticle);
  ...
  it('keeps Reading Mode open when chapter navigation targets an unsupported URL', async () => {
    ...
    const result = await workflow.navigateReadingChapter('next');

    expect(result.ok).toBeFalse();
    expect(workflow.readingArticle()).toEqual(article);
    expect(viewport.loadedUrls).toEqual([]);
    expect(workflow.notice()).toEqual({
      kind: 'unsupportedCapability',
      message: 'Only HTTP and HTTPS links are supported.',
  ...
  it('disables concurrent chapter navigation while a chapter is loading', async () => {
    ...
    const blockedResult = await workflow.navigateReadingChapter('previous');
    extraction.resolve({
      status: 'ok',
      article: articleSnapshot,
    });
  ...
  it('ignores chapter navigation when the direction is unavailable', async () => {
    ...
    const result = await workflow.navigateReadingChapter('previous');
